...
from catalog.services.media import wav_duration
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser) -> None:
        parser.add_argument(
            "--reset", action="store_true", help="Delete existing Sonora data before seeding."
        )
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        from billing.services.accounting import sync_artist_ledger

        sync_artist_ledger()
        self.stdout.write(
            self.style.SUCCESS("Sonora demo data is ready. Demo password: DemoPass123!")
        )
    ...
    def seed_users(self) -> dict[str, User]:
        rows = [
            ...
            (
                ...
                User.Kind.CONSUMER,
                "basic",
                "/media/avatars/user-artist-u.svg",
            ),
            (
                "artist_v",
                ...
                User.Kind.CONSUMER,
                "gold",
                "/media/avatars/user-artist-v.svg",
            ),
            (
                "artist_2",
                "lumen@sonora.demo",
                ...
                User.Kind.CONSUMER,
                "silver",
                "/media/avatars/user-artist-2.svg",
            ),
            (
                "artist_3",
                "sol@sonora.demo",
                ...
                User.Kind.CONSUMER,
                "gold",
                "/media/avatars/user-artist-3.svg",
            ),
            (
                "artist_4",
                "mira@sonora.demo",
                ...
                User.Kind.CONSUMER,
                "silver",
                "/media/avatars/user-artist-4.svg",
            ),
            (
                "support",
                "support@sonora.demo",
    ...
    def seed_artist_profiles(self, users: dict[str, User]) -> dict[str, ArtistProfile]:
        rows = {
            "artist_u": (
                "Cedar Bloom",
                "Bedroom producer shaping quiet electronic sketches.",
                "Ambient",
                None,
            ),
            "artist_v": (
                "Nova Serein",
                "Luminous electronica assembled from night trains and warm circuitry.",
                "Electronic",
                "2025-01-14T10:00:00Z",
            ),
            "artist_2": (
                "Lumen Harbor",
                "Organic textures and patient percussion.",
                "Downtempo",
                "2025-02-02T10:00:00Z",
            ),
            "artist_3": (
                "Sol Circuit",
                "Bright synth music for long roads.",
                "Synthwave",
                "2025-03-02T10:00:00Z",
            ),
            "artist_4": (
                "Mira Moss",
                "Acoustic fragments with an electronic pulse.",
                "Indie",
        ...
        for label, (stage, bio, genre, verified) in rows.items():
            profile, _ = ArtistProfile.objects.update_or_create(
                ...
                defaults={
                    ...
                    "bio": bio,
                    "genre": genre,
                    "verified_at": dt(verified) if verified else None,
                },
            )
    ...
    def seed_plans(self) -> None:
        for tier, monthly in [
            (SubscriptionPlan.Tier.SILVER, 790_000),
            (SubscriptionPlan.Tier.GOLD, 1_290_000),
        ]:
        ...
        for label, tier in tiers.items():
            Subscription.objects.update_or_create(
                ...
                defaults={
                    ...
                    else dt("2027-07-06T08:00:00Z"),
                },
            )

    def seed_catalog(
        self, users: dict[str, User], profiles: dict[str, ArtistProfile]
    ) -> dict[int, Track]:
        releases = [
            (
                1,
        ...
        track_rows = [
            ...
            (
                ...
                48,
                True,
                "Lines of light / a borrowed city\nWe draw the dark in symmetry",
                22_260,
                8_018,
            ),
        ...
        for (
            ...
            track, _ = Track.objects.update_or_create(
                ...
                defaults={
                    ...
                    "title": title,
                    "processed_audio": f"audio/processed/sonora-{number}.wav"
                    if target.exists()
                    else "",
            ...
            TrackArtistCredit.objects.update_or_create(
                id=uid(f"credit-{number}"),
                defaults={
                    "track": track,
                    "artist": profiles[artist_label],
                    "role": TrackArtistCredit.Role.PRIMARY,
                },
            )
            tracks[number] = track
        return tracks
    ...
    def seed_social_library(self, users: dict[str, User], tracks: dict[int, Track]) -> None:
        ...
        for user_label, track_numbers in [
            ("basic", [1, 5]),
            ("silver", [2, 6]),
            ("gold", [1, 5, 12]),
        ]:
        ...
        for label, numbers in [("basic", [3, 1, 8]), ("gold", [12, 1])]:
            for index, number in enumerate(numbers):
                RecentlyPlayed.objects.update_or_create(
                    id=uid(f"recent-{label}-{number}"),
                    defaults={
                        "user": users[label],
                        "track": tracks[number],
                        "played_at": timezone.now() - timezone.timedelta(minutes=index),
                    },
                )
        ...
        playlist_rows = [
            (
                ...
                "artist_v",
                "Night Transit",
                "Glowing tracks for the last train home.",
            ...
            (
                ...
                "gold",
                "Low light focus",
                "Patient sounds, minimal interruption.",
                Playlist.Visibility.PUBLIC,
                [7, 2, 10],
            ),
            (
        ...
        for key, owner, title, desc, visibility, numbers in playlist_rows:
            ...
            PlaylistItem.objects.filter(playlist=playlist).exclude(
                track__in=[tracks[number] for number in numbers]
            ).delete()
            for pos, number in enumerate(numbers):
                PlaylistItem.objects.update_or_create(
                    playlist=playlist, track=tracks[number], defaults={"position": pos}
                )

    def seed_workflows(self, users: dict[str, User], profiles: dict[str, ArtistProfile]) -> None:
        Notification.objects.update_or_create(
            id=uid("notice-gold-early"),
            defaults={
                "user": users["gold"],
                "title": "Future Bloom is here early",
                "body": "Your Gold early-access window is open.",
                "title_key": "noticeGoldEarlyTitle",
                "body_key": "noticeGoldEarlyBody",
                "values": {"releaseTitle": "Future Bloom"},
                "kind": Notification.Kind.RELEASE,
            },
        )
        Notification.objects.update_or_create(
            id=uid("notice-basic-security"),
            defaults={
                "user": users["basic"],
                "title": "Account protected",
                "body": "Your security settings are up to date.",
                "title_key": "noticeAccountProtectedTitle",
                "body_key": "noticeAccountProtectedBody",
                "kind": Notification.Kind.CRITICAL,
            },
        )
        pending, _ = ArtistVerificationRequest.objects.update_or_create(
            id=uid("verify-pending"),
            defaults={
                "artist": profiles["artist_u"],
                "portfolio_urls": ["https://example.com/cedar-bloom"],
                "note": "Independent producer and live performer.",
                "status": ArtistVerificationRequest.Status.PENDING,
            },
        )
        approved, _ = ArtistVerificationRequest.objects.update_or_create(
            id=uid("verify-approved"),
            defaults={
                "artist": profiles["artist_v"],
                "portfolio_urls": ["https://example.com/nova-serein"],
                "note": "Official portfolio.",
                "reason": "Identity and catalog confirmed.",
                "status": ArtistVerificationRequest.Status.APPROVED,
                "reviewer": users["support"],
                "decided_at": dt("2025-01-14T10:00:00Z"),
            },
        )
        ticket, _ = Ticket.objects.update_or_create(
            id=uid("ticket-1"),
            defaults={
                "creator": users["silver"],
                "subject": "Downloaded track unavailable",
        ...
        TicketMessage.objects.update_or_create(
            ...
            defaults={
                "ticket": ticket,
                "author": users["silver"],
                "body": "The download action stays disabled on one release.",
            },
        )
    ...
    def seed_payments(self, _users: dict[str, User]) -> None:
        from billing.services.accounting import ensure_subscription_payments

        ensure_subscription_payments()
