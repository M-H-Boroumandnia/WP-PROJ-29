...
def room_state(invite_code: str) -> dict:
    room = ListeningRoom.objects.select_related("host", "current_queue_item__track").get(
...
def can_read_room(user: User, invite_code: str) -> bool:
    return ListeningRoom.objects.filter(
        invite_code=invite_code, participants__user=user, participants__left_at__isnull=True
    ).exists()
...
def mutate_room(user: User, invite_code: str, action: str, payload: dict) -> str:
    with transaction.atomic():
        ...
        participant = (
            RoomParticipant.objects.select_for_update()
            .filter(room=room, user=user, left_at__isnull=True)
            .first()
        )
        if not participant:
        ...
        if action == "play":
            ...
            if room.host_id != user.id:
                raise SonoraError(
                    "host_required", "Only the host can change controller permissions."
                )
            target = RoomParticipant.objects.get(
                room=room, user_id=payload.get("userId"), left_at__isnull=True
            )
    ...
    async def receive_json(self, content: dict, **kwargs) -> None:
        ...
        try:
            ...
            await self.send_json(
                {"type": "error", "error": {"code": exc.code, "message": str(exc)}}
            )
        except Exception:
            await self.send_json(
                {"type": "error", "error": {"code": "room_error", "message": "Room event failed."}}
            )

    async def broadcast_state(self, event_type: str = "room_state") -> None:
        state = await room_state(self.invite_code)
        await self.channel_layer.group_send(
            self.group_name, {"type": "room.event", "event": {"type": event_type, "room": state}}
        )
